...
turma: Turma
turma = [ ("Joao",7,4), ("Maria",10,8), ("Joaquim", 4, 9), ("Manelito", 9, 2), ("Reripoterson", 8, 6) ]       -- 50 alunos
...
estaAprovado: Aluno -> Bool
estaAprovado aluno = (media aluno) >= 7

aprovados: List String
aprovados = List.map nome (List.filter estaAprovado turma)
...
calculaMedia: List Aluno -> Float
calculaMedia alunos = (List.foldl somaMedias 0 alunos) / (toFloat (List.length alunos))

total: Float
total = calculaMedia turma
...
mediaAprovados turma = List.map listaMedia (List.filter estaAprovado turma)
